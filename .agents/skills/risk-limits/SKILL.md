...
| Liquidity-based position cap | Full position exitable within 2× the tier slippage cap | Estimated exit slippage for the entire position, using **gatekeeper** depth data for tokens and visible order-book depth otherwise, against the liquidity tiers in **execution** |
