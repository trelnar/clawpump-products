...
- **Rug filter** (`rugcheck.py`, operator "Build it" 2026-09-13). Three of the first five
  exits were rugs (−100%, −99% six minutes after the buy, −71%) on tokens that cleared the
  old $5k liquidity bar; a stop cannot fire through a pulled pool. A token is bought only
  with ≥ $30k liquidity, a pair older than 1h, off the bonding curve (`pumpfun`,
  `moonshot`), and, on Solana, the top 10 wallets holding under 40% of supply (AMM vaults
  excluded; an unreadable holder list is a refusal, not a pass). The pair checks also run
...
| ~~**Strategy inputs**~~ | *Fixed 2026-09-05.* Discovery was two *paid-promotion* endpoints plus an alphabetical slice of Coinbase products. It is now the signal layer (`SIGNALS.md`): launches, graduations, new pools, Reddit, Farcaster, holder growth, Telegram calls — ranked by acceleration × breadth. Paid promotion is off by default. A 12-finding adversarial review of the layer is applied (first-seen backdating, single-source scoring cap, per-source time budget, Base id case, pump.fun query, tgmon login/loop, pool-id extraction). **Unverified live**: the sandbox could not reach any source, so parsers are fixture-tested only — run `scripts/signals_probe.py` after deploying and look at its per-kind table. Still open: only 3 of 18 skills reach the model. |
